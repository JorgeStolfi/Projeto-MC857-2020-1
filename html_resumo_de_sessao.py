...
def gera(ses, bt_ver, bt_fechar):
  ...
  sessão: identificador da sessão, usuário dono da sessão,
  um booleano indicando se a sessão está aberta ou não,
  o cookie da sessão e também o identificador do carrinho
  associado à sessão. Além disso, botões para ver detalhes
  da sessão e para fechá-la (caso esteja aberta) também são retornados,
  de acordo com os valores dos parâmetros {bt_ver} e {bt_fechar}
